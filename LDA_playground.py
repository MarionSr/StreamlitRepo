        ...
        if filename.find(interview_topic) != -1:
            ...
            with open(os.path.join(doc_path,filename), 'r') as my_file:
                ...
                if mode == 'lemmization':
                    ...
                    break
    print(f'Found {len(list_interview_per_topic)} texts for topic {interview_topic}.')
...
def compute_coherence_values(dictionary, corpus, texts, limit, start, step):
    ...
    for num_topics in range(start, limit, step):
        print(f'Computing LDA with {num_topics} topics…')
    ...
    pd.DataFrame({'coherence values': coherence_values}, index=number_topics).plot()
...
def main():
    ...
    ## Verbose mode
    f'Dictionary using mode contains {count} interview parts related to {len(dict_topics)} topics.'
    ...
    pyLDAvis.gensim_models.prepare(lda_model, corpus, dictionary, mds='mmds')
